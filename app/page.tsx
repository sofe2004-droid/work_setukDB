...
export default function Home() {
  ...
  const [apiKey, setApiKey] = useState(""); const [model, setModel] = useState(DEFAULT_MODEL); const [outputCount, setOutputCount] = useState("1");
  ...
  const [showHistory, setShowHistory] = useState(false); const [databaseStatus, setDatabaseStatus] = useState("연결 확인 전");
  ...
  async function checkDatabase() { if (!supabase) return setDatabaseStatus("환경 변수 미설정"); setDatabaseStatus("연결 확인 중…"); const { error } = await supabase.from("setuk_drafts").select("id", { head: true, count: "exact" }).limit(1); setDatabaseStatus(error ? `연결 오류: ${error.message}` : "연결됨"); }
  ...
  return <main className="studio"><header className="brand"><div className="brand-mark">✦</div><b>세특 스튜디오</b><button className="history-link" onClick={loadHistory}>저장 내역</button><span>3-AGENT WORKFLOW</span></header><section className="hero"><h1>관찰을 정리하고,<br />교과 세특 초안으로.</h1><p>수집 · 작성 · 검토 에이전트가 순서대로 협업합니다. 교사의 최종 확인을 위한 초안 도구입니다.</p></section>
    ...
    {message && <p className="notice">{message}</p>}<p className="db-status">DB 상태: <b>{databaseStatus}</b><button onClick={checkDatabase}>다시 확인</button></p><section className="workspace"><section className="card input-card"><h2>학생 활동 입력</h2><label>학생 식별값 <small>학번 또는 개인정보를 제외한 별칭</small><input value={studentId} onChange={(e) => setStudentId(e.target.value)} placeholder="예: 10101 또는 학생A" /></label><div className="dual"><label>학년<select value={grade} onChange={(e) => setGrade(e.target.value)}><option>1학년</option><option>2학년</option><option>3학년</option></select></label><label>결과물 개수<select value={outputCount} onChange={(e) => setOutputCount(e.target.value)}><option value="1">1개</option><option value="2">2개</option><option value="3">3개</option></select></label></div><label>과목 <small>쉼표로 여러 과목 입력 가능</small><input value={subjectsText} onChange={(e) => setSubjectsText(e.target.value)} placeholder="예: 국어, 과학" /></label><div className="quick-subjects">{quickSubjects.map((subject) => <button key={subject} onClick={() => chooseSubject(subject)}>{subject}</button>)}</div><label>학생 활동 키워드 또는 관찰 내용<textarea rows={7} value={observation} onChange={(e) => setObservation(e.target.value)} placeholder="수업에서 관찰한 활동, 과정, 질문, 산출물을 구체적으로 입력하세요." /></label><button className="generate" disabled={busy} onClick={generate}>{busy ? "에이전트가 작성 중입니다…" : "3개 에이전트로 초안 만들기 →"}</button></section>
